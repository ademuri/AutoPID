/*
   ...
   This program reads a dallas temperature probe as input, potentiometer as
   setpoint, drives an analog output. It lights an LED when the temperature has
...
AutoPID myPID(&temperature, &setPoint, &outputVal, OUTPUT_MIN, OUTPUT_MAX, KP,
              KI, KD);
...
void loop() {
  ...
  myPID.run();  // call every loop, updates automatically at certain time
                // interval

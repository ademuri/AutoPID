...
class AutoPID {
 public:
  ...
  AutoPID(float *input, float *setpoint, float *output, float outputMin,
...
class AutoPIDRelay : public AutoPID {
 public:
  AutoPIDRelay(float *input, float *setpoint, bool *relayState,
               float pulseWidth, float Kp, float Ki, float Kd)
      : AutoPID(input, setpoint, /*output=*/&_pulseValue, /*outputMin=*/0,
